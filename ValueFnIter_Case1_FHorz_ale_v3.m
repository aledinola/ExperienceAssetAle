function [V,Policy] = ValueFnIter_Case1_FHorz_ale_v3(n_d,n_a,n_z,N_j, d_grid, a_grid, z_grid, pi_z, ReturnFn, Params, DiscountFactorParamNames,XXX, vfoptions,par)
%% VECTORIZED GPU
% STATE VARIABLES
% V(a,h,z,j)
% a: asset holdings
...
V_d = gpuArray(V_d);
Pol_aprime_d= gpuArray(Pol_aprime_d);

eta_m_val = shiftdim(z_grid(:,1),-3);
eta_f_val = shiftdim(z_grid(:,2),-3);
theta_val = shiftdim(z_grid(:,3),-3);
eta_m_val = gpuArray(eta_m_val);
eta_f_val = gpuArray(eta_f_val);
theta_val = gpuArray(theta_val);
...
for j = N_j-1:-1:1
    ...
    z_prob = shiftdim(pi_z',-2); %(1,1,z',z)
    EV = V_next.*z_prob; %V(a',h',z')*Prob(1,1,z',z)
    EV = sum(EV,3); %EV(a',h',1,z)

    for d_c=1:n_d
        d_val = d_grid(d_c);
        % aprime_val is (a',1),a_val is (1,a), h_grid3 is (1,1,h) and
        % z_val is (1,1,1,z) --> Ret_mat is (a',a,h,z)
        ...
        % entireRHS is (a',a,h,z) since Ret_mat is (a',a,h,z) and
        % permute(EV_interp,[1,4,2,3]) is (a',1,h,z)
        entireRHS = Ret_mat+beta*s_j*permute(EV_interp,[1,4,2,3]);
        [max_val,max_ind] = max(entireRHS,[],1);
        % max_val and max_ind are (1,a,h,z)
        V_d(:,:,:,d_c) = max_val;  %best V given d
        Pol_aprime_d(:,:,:,d_c) = max_ind; %best a' given d, indexes
    end %end d
    ...
    %     end
    % end
    Pol_aprime_d2 = reshape(Pol_aprime_d,[n_a*n_h*n_z,n_d]);
    d_star_lind = sub2ind([n_a*n_h*n_z,n_d],(1:n_a*n_h*n_z)',d_max(:));
    Policy(2,:,:,:,j) = reshape(Pol_aprime_d2(d_star_lind),[n_a,n_h,n_z]);
